...
```{r echo=FALSE}
census_api_key("0a531494f1ff2d7f47e0a5622be35c2815f32c4a", install=TRUE, overwrite = TRUE)
```
...
**Task** Run the following codeblock to create a chart showing the margin of error for each county. The current headline isn't very descriptive of the data. What should it be? **Answer: Median household income by Maryland counties**
...
Is the difference between Calvert and Montgomery significant? **Answer: No because Calvert County has a big margin of error but in reality there is only a few thousand dollar difference between the median income in both counties.**

Is the difference between Howard and everyone else significant? **Answer: Yes because even while considering the margin of error the median income in Howard county will be over** \$120,000. Only Montgomery County could have a median of income that high depending on the size of the margin error. After the top three counties the median household income is \$110,000 or less which is a big drop off from \$120,000 and \$130,000.
