...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with Y. Name a problem that you see that is preventing proper grouping. **Answer: There are a couple of places with similar names but have slightly different format with different letters capitalized or lower case or include INC after the name.**
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with Y. Name three changes the refinr function made in the first 30 results. **Answer: Different versions of Y of Central Maryland were all grouped as Y OF CENTRAL MARYLAND in the grantee_clean column. Variations of Yeshiva of Greater Washington was changed to YESHIVA OF GREATER WASHINGTON.**
...
Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with Y. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer: No, for our slice of the data is to small to pick up any changes.**
