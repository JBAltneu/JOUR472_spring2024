...
overdose_calls <- read_csv("data/tabula-Baltimore County; Carey, Samantha log OD.csv" , col_names = FALSE) |>
  clean_names() |>
  rename(date = x1, time = x2, case_number = x3, calls = x4, location = x5)
...
A1. July 14, 2022 had the most overdose calls with 23. I believe there were no days in 2022 that did not have any overdose calls since there are 366 different rows in the new table. There should be at least one duplicate date entered in the chart.
...
**Note: I got some help from my classmates but got stuck, that's why I did not finish this question.**

```{r}
overdose_calls <- overdose_calls |> 
  mutate(day_of_week = wday(date)) 
  
  #weekly_overdose_calls <- overdose_calls |>
  #group_by(day_of_week) |>
  #summarize (day_of_week = n()) |>
  #mutate(pct_total = (day_of_week/sum(day_of_week) *100))
...
A3. The second and third most common location are at Precincts which are police stations. Are the police officers the one overdosing or is it people that are arrested that overdose while they are in custody. Additionally, the most common address appears to be a house which is interesting since the house seemed pretty small and there were 36 calls for that location.

```{r}
overdose_calls |>
  group_by(location) |>
  summarize (
    count_of_locations = n()
  ) |>
  arrange(desc(count_of_locations))
```

Q4. What's the best story idea or question you've seen as a result of the work you've done in this lab?

A4. Why did the house at 4540 Silver Spring Road have 36 overdose calls? Were they all on one or two days as a result of a party? What was the reason?
