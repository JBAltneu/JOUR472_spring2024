...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer: Some people might have entered symbols or letters after the zip code making Vendor Zip a character instead of a number**
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think is happening with this data? **Answer: There are 549 different invalid zip codes that appear many times. Maybe people entered the zip code wrong. There are also some zip codes with letters in it. Letters do not represent zip codes at all.**
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer: Some categories don't have an entry making it appear as N/A in the category column but there were very few of those. Additionally, there are way more grants handed out than loans.**
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer: The Land Preservation Trust, Inc. received a grant of \$60, the smallest amount in the data set.**
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer: 17,740 entries**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer: Only shows the first five numbers of the zip code.**
...
If I wrote the code correctly then the zip code 21202 received \$8,464,101 from the Maryland Tourism Board. That is the area near Baltimore's Inner Harbor which is a major tourist attraction in Maryland
...
#class answer
#answer <- cleaned_md_grants_loans |>
  #filter(str_detect(source, 'MARYLAND TOURISM BOARD'))
  #group_by(zip5) |>
  #summarise(total_amount = sum(amount)) |>
  #arrange(desc(total_amount))
