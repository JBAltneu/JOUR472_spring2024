...
```{r}
pg_overdose_2022 <- read_csv("data/prince_georges_2022_overdoses.csv")

pg_overdose_2022 <- pg_overdose_2022 |>
...
```{r}
pg_overdose_calls_per_month <- pg_overdose_2022 |>
  group_by(month) |>
  summarize(total_calls = n()) |>
  mutate(
    pct_total = (total_calls / 1397) *100 
  )
```

```{r}
pg_overdose_calls_per_month |>
  ggplot() +
  geom_bar(aes(x= month, weight = pct_total)) +
  coord_flip() +
  labs (
    title = 'One in Ten Overdose Calls in Prince George\'s county occured in December 2022',
    x = 'month',
    y = 'percent of calls',
...
```{r}
pg_calls_by_week <- pg_overdose_2022 |>
  group_by(week) |>
  summarise(total_calls_per_week = n())
```

```{r}
pg_calls_by_week |>
  ggplot() +
   geom_line(aes(x = week, y= total_calls_per_week)) +
   # scale_x_date(date_breaks = "1 week") +
    labs (
    title = 'Number of overdose calls per week in Prince George\'s county in 2022',
    x = 'week',
    y = 'number of calls',
    caption = 'Source: PG County EMS'
  )
...
1.  Generate a dataframe with the number of Prince George's 911 calls for each zip code (same data we used earlier in the lab).
...
```{r}
pg_calls_zipcodes <- pg_overdose_2022 |>
  group_by(zipcode) |>
  summarize(total_calls = n())
```
...
#Step 3 
pg_overdoses_with_income <- pg_calls_zipcodes |>
  inner_join(md_median_income_zip, by = c('zipcode' = 'GEOID'))
```

```{r}
#steps 4-7
  ggplot(pg_overdoses_with_income, aes(x= estimate, y = total_calls)) +
    geom_point() + 
    labs (
      title = 'Prince George\'s median income to number of overdose calls',
      x = 'Median income',
