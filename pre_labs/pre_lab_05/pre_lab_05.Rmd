...
```{r}
fentanyl_deaths <- read_csv("tabula_Annual_2020_Drug_Intox_Report.csv") |> clean_names()
...
**Task** Load the data without headers by running the codeblock below. What problems remain? **Answer: Each column as an x then the number for what column it is**
...
**Task** Run the codeblock below. What else do we need to do? **Answer: Get rid of the x and number as the name for each column name.**

```{r}
fentanyl_deaths <- read_csv("tabula_Annual_2020_Drug_Intox_Report.csv", col_names = FALSE) |> 
  clean_names() |> 
